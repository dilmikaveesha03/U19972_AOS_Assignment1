...
#University Data Center Process & Resource Management System
...
CRITICAL_PROCESSES=("init" "systemd" "kthreadd" "kworker" "kksoftirqd" "migration" "rcu_" "watchdog")
...
monitor_system_resources() {
	...
	echo ""
	echo "___ CPU Usage (top snapshot) ___"
	top -bn1 | head -5

	echo ""
	echo "___ Memory Usage (free -h) ___"
	free -h

	echo ""
	echo "___Virtual Memory Statistics (vmstst) ___"
	vmstat 1 1

	log_action "View system resources" \
		"Displayed CPU, memory, and vmstat output"
...
manage_processes() {
	...
	echo ""
	echo "___ Top 10 Memory-Consuming Processes ___"
	echo ""
...
if ! [[ "$pid" =~ ^[0-9]+$ ]] || [ "$pid" -eq 0 ]; then
	...
	log_action "Terminate process" \
		"User cancelled or invalid PID entered"
...
for critical in "${CRITICAL_PROCESSES[@]}"; do
	if [[ "$proc_name" == *"$critical"* ]]; then
		...
		log_action \
			"Attempt to terminate critical process '$proc_name' (PID $pid)" \
			"BLOCKED - critical process protecttion"
...
if [[ "$confirm" =~ ^[Yy]$ ]]; then
	...
	if [ $? -eq 0 ]; then
		...
		log_action \
			"Terminate process '$proc_name' (PID $pid)" \
			"Success - SIGTERM sent"
				else
...
inspect_disk_usage() {
	...
	if [ ! -d "$dir_path" ]; then
		...
		log_action \
			"Inspect disk usage of '$dir_path'" \
			"Failed - directoty not found"
	...
	echo ""
	echo "___Overall Disk Usage (df -h) ___"

	df -h "$dir_path"

	echo ""
	echo "___ Directory Size (du -sh) ___"

	du -sh "$dir_path"

	echo ""
	echo "___ Top 10 Largest Subdirectories ___"

	du -h --max-depth=1 "$dir_path" 2>/dev/null |
		sort -rh |
		head -10

	log_action \
		"Inspect disk usage of '$dir_path'" \
		"Displayed df and du output"
...
archive_large_logs() {
	...
	read -rp " Enter directory to scan for large files: " scan_dir
	...
	if [ ! -d "$scan_dir" ]; then
		...
		log_action \
			"Scan for large logs in '$scan_dir'" \
	...
	if [ ! -d "$ARCHIVE_DIR" ]; then
		...
		log_action \
			...
			"Created at $ARCHIVE_DIR"
	fi

	echo ""
	echo "___ Scanning '$scan_dir' for log files latger than 50 MB ___"
	echo ""
	...
	if [ ${#found_files[@]} -eq 0 ]; then
		...
		echo " [INFO] Nolog files larger then 50 MB found."

		log_action \
			"Scan for large logs in '$scan_dir'" \
			"No oversized log files found"
				else
					...
					if [[ "$confirm_archive" =~ ^[Yy]$ ]]; then
						...
						log_action \
							"Archive large logs in '$scan_dir'" \
							"Cancelled by user"
					fi
	fi
	...
	if [ "$archive_size" -gt "$ARCHIVE_SIZE_LIMIT" ]; then
		...
		log_action \
			...
			"WARNING: Size exceeds 1 GB ($archive_human)"
	fi
...
main_menu() {
	...
	while true; do
		...
		echo ""
		echo " =============================================="
		echo "    University Data Center Management System   "
		echo " =============================================="
		...
		echo ""
		read -rp " Select an potion [1-5]: " choice
		...
		case "$choice" in
			...
			*)
				...
				log_action \ 
					"Menu input '$choice'" \
					"Invalid option - prompt re-displayed"
...
log_action \
	"System started" \
	"Data Center Management System initialised by user: $(whoami)"
